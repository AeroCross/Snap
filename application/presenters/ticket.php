	...
	public function files($ticket_id) {
		...
		foreach($results as $result) {
			...
			$this->app->table->add_row(
				anchor('file/get/tickets/' . $ticket_id . '/' . $result[0] . '/' . $file, img($this->app->resource->img(extension($ext))), array('class' => 'file-extension')) . ' ' . anchor('file/get/tickets/' . $ticket_id . '/' . $result[0] . '/' . $file, $file),
				extension_name($ext),
